...
def WaveletFusion(C1, C2):
    ...
    x, y = C1.shape
    C = [[0 for a in range(x)] for a in range(y)]
    for i in range(x):
        for j in range(y):
...
if __name__ == "__main__":
    ...
    for i in range(0, 3):
        wavelet_signal = np.array(fwt97_2d(im1_matrix[i], 3))
        wavelet_signal2 = np.array(fwt97_2d(im2_matrix[i], 3))
        result_fusion = WaveletFusion(wavelet_signal, wavelet_signal2)
        image_channel = iwt97_2d(result_fusion, 3)
        final_im_channels[:, :, i] = image_channel
